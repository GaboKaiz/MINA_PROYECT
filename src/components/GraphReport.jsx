...
function GraphReport() {
  ...
  const id_equipo = 1; // 👈 Cambia este valor por el ID real que necesites

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await getVitalSignsForGraph(id_equipo);
    ...
    datasets: [
      {
        ...
        borderColor:
          chartType === 'pie' || chartType === 'doughnut'
            ? ['#4285f4', '#34c759', '#fbbc05', '#ea4335', '#00c4b4']
            : '#4285f4',
        backgroundColor:
          chartType === 'pie' || chartType === 'doughnut'
  ...
  return (
    <div className="bg-white p-5 rounded-lg shadow-md">
      <h2 className="text-2xl font-bold text-gray-800 mb-5">
        Reporte Gráfico Filtrado por Sensor
      </h2>
